...
//     constructor
    ...
    public void enableSearchInDescriptions() {
        ...
        By descCheckbox = By.name("description");

        // 1️⃣ احصل على العنصر الجديد (مش القديم)
        WebElement checkbox = wait.until(
                ExpectedConditions.refreshed(
                        ExpectedConditions.elementToBeClickable(descCheckbox)
                )
        );

        // 2️⃣ لو مش متعلم → اعمله click
        if (!checkbox.isSelected()) {
            checkbox.click();
        }

        // 3️⃣ انتظر تحميل النتائج الجديدة بعد تغيير الـ filter
    ...
    public void setShowCount(int count) {
        ...
        // select dropdown
        WebElement dropdown = wait.until(ExpectedConditions.elementToBeClickable(By.id("input-limit")));

        Select select = new Select(dropdown);
        select.selectByVisibleText(String.valueOf(count));

        // After selecting, page reloads → wait for URL update
        wait.until(ExpectedConditions.urlContains("limit=" + count));
    }
